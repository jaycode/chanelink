  ...
  def create_hotel_inventory_xml(xml, room_type_logs, channel_room_type_map)
    room_type_logs.each do |log|
      xml.AvailStatusMessage(:BookingLimit => log.total_rooms, :BookingLimitMessageType => "SetLimit") {
        xml.StatusApplicationControl(:RatePlanCategory => channel_room_type_map.ctrip_room_rate_plan_category, :RatePlanCode => channel_room_type_map.ctrip_room_rate_plan_code, :Start => date_to_key(log.inventory.date), :End => date_to_key(log.inventory.date)) {
