...
class RoomType < ActiveRecord::Base
  ...
  def calculated_inventory(date, pool, flash)
    ...
    if self.is_inventory_linked?
      ...
      inv = Inventory.find_by_date_and_property_id_and_pool_id_and_room_type_id(date, self.property.id, pool.id, linked.id)
      ...
      if flash[:inventory] and flash[:inventory][self.id.to_s]
        ...
        inv = Inventory.find_by_date_and_property_id_and_pool_id_and_room_type_id(date, self.property.id, pool.id, self.id)
        result = inv.total_rooms unless inv.blank?
      end
    end
    result
