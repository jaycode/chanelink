...
# class to handle Orbitz push for Min Stay
class OrbitzChannelMinStayHandler < ChannelMinStayHandler
        ...
        xml.AvailStatusMessages(:ChainCode => property_channel.orbitz_chain_code, :HotelCode => property_channel.orbitz_hotel_code) {
          logs_to_use.each do |log|
            ...
            xml.AvailStatusMessage {
              xml.StatusApplicationControl(:Start => date_to_key(channel_min_stay.date), :End => date_to_key(channel_min_stay.date), :InvCode => channel_room_type_map.orbitz_room_type_id, :InvCodeApplication => "InvCode", :RatePlanCode => channel_room_type_map.orbitz_rate_plan_id)
              xml.LengthsOfStay {
                xml.LengthOfStay(:MinMaxMessageType => "SetMinLos", :Time => log.min_stay, :TimeUnit => "Day")
              }
            }
          end
        }
