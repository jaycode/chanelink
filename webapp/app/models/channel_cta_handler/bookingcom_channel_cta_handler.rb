...
# class to handle Booking.com XML push for CTA
class BookingcomChannelCtaHandler < ChannelCtaHandler
        ...
        logs_by_room_type.keys.each do |rt_id|
          ...
          xml.room(:id => channel_room_type_map.bookingcom_room_type_id) {
            room_type_logs.each do |log|
              xml.date(:value => date_to_key(log.channel_cta.date)) {
                xml.rate(:id => channel_room_type_map.bookingcom_rate_plan_id)
                xml.closedonarrival RateUtils.convert_to_int(log.cta)
              }
            end
          }
