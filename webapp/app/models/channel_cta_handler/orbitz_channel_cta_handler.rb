...
# class to handle Orbitz push for CTA
class OrbitzChannelCtaHandler < ChannelCtaHandler
        ...
        xml.AvailStatusMessages(:ChainCode => property_channel.orbitz_chain_code, :HotelCode => property_channel.orbitz_hotel_code) {
          logs_to_use.each do |log|
            ...
            xml.AvailStatusMessage {
              xml.StatusApplicationControl(:Start => date_to_key(channel_cta.date), :End => date_to_key(channel_cta.date), :InvCode => channel_room_type_map.orbitz_room_type_id, :InvCodeApplication => "InvCode", :RatePlanCode => channel_room_type_map.orbitz_rate_plan_id)
              xml.RestrictionStatus(:Restriction => "Arrival", :Status => (channel_cta.cta? ? "Open" : "Closed"))
            }
          end
        }
      }
