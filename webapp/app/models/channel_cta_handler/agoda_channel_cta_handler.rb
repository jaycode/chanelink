...
# class to handle AgodaXML push for CTA
class AgodaChannelCtaHandler < ChannelCtaHandler
          ...
          change_set.logs.each do |log|
            ...
            if room_type_ids.include?(room_type.id)
              xml.HotelInventory {
                ...
                xml.DateRange(:Type => "Stay", :Start => date_to_key(channel_cta.date), :End => date_to_key(channel_cta.date))
                xml.InventoryAllotment {
                  xml.ClosedToArrival log.cta
                }
              }
            end
          end
