...
# class to handle Orbitz push for Stop Sell
class OrbitzChannelStopSellHandler < ChannelStopSellHandler
        ...
        xml.AvailStatusMessages(:ChainCode => property_channel.orbitz_chain_code, :HotelCode => property_channel.orbitz_hotel_code) {
          logs_to_use.each do |log|
            ...
            xml.AvailStatusMessage {
              xml.StatusApplicationControl(:Start => date_to_key(channel_stop_sell.date), :End => date_to_key(channel_stop_sell.date), :InvCode => channel_room_type_map.orbitz_room_type_id, :InvCodeApplication => "InvCode", :RatePlanCode => channel_room_type_map.orbitz_rate_plan_id)
              xml.RestrictionStatus(:Restriction => "Master", :Status => (channel_stop_sell.stop_sell? ? "Open" : "Closed"))
            }
          end
        }
      }
