...
class PopulateRackRateController < ApplicationController
      ...
      property.pools.each do |pool|
        PropertyChannel.find_all_by_pool_id(pool.id).each do |pc|
          ...
          RoomTypeChannelMapping.room_type_ids(property.room_type_ids).where(:channel_id => channel.id, :disabled => false).each do |rtm|
            populate_room_type_channel(rtm.room_type, rtm.rate_type, channel, pool, property)
          end
  ...
  def populate_room_type_channel(room_type, rate_type, channel, pool, property)
    if !RoomTypeMasterRateChannelMapping.find_by_room_type_id_and_rate_type_id_and_channel_id(room_type.id, rate_type.id, channel.id).blank?
      ...
      while loop_date <= Constant.maximum_end_date
        ...
        existing_rate = ChannelRate.find_by_date_and_property_id_and_pool_id_and_room_type_id_and_rate_type_id_and_channel_id(
          loop_date, property.id, pool.id, room_type.id, rate_type.id, channel.id)
  ...
  def populate_master_room(master_room, pool, property)
    ...
    rate_type = master_room.rate_type
    logs = Array.new
    loop_date = DateTime.now.in_time_zone.beginning_of_day
    ...
    while loop_date <= Constant.maximum_end_date
      ...
      existing_rate = MasterRate.find_by_date_and_property_id_and_pool_id_and_room_type_id_and_rate_type_id(
        loop_date, property.id, pool.id, room_type.id, rate_type.id)
